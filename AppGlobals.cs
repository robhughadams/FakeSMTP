...
namespace FakeSMTP
{
	public static class AppGlobals
	{
		...
		// sessions count
		private static readonly object _lkSessions = new object();
		private static long _sessions;

		private static readonly object _lkSessId = new object();
		private static long _sessId;

		private static readonly object _lkAppLog = new object();
		private static readonly object _lkSesLog = new object();
		...
		public static void LogMessage(string format, params object[] args)
		{
			lock (_lkAppLog)
			{
				...
				{
					Debug.WriteLine(format, args);
					var logFile = _logPath + "fakesmtp-" + DateTime.UtcNow.ToString("MM") + ".log";
					_RollFile(logFile);
					var fp = new StreamWriter(logFile, true);
		...
		public static void LogSession(string format, params object[] args)
		{
			lock (_lkSesLog)
			{
				...
				{
					Debug.WriteLine(format, args);
					var logFile = _logPath + "smtpsess-" + DateTime.UtcNow.ToString("MM") + ".log";
					_RollFile(logFile);
					var fp = new StreamWriter(logFile, true);
					fp.WriteLine(format, args);
					fp.Flush();
					fp.Close();
				}
				catch (Exception ex)
		...
		public static List<string> LoadFile(string fileName)
		{
			...
			try
			{
				var fp = new StreamReader(fileName);
				string buffer;
		...
		private static void _RollFile(string pathName)
		{
			try
			{
				if (!File.Exists(pathName))
					return;
				var lastWrite = File.GetLastWriteTime(pathName);
